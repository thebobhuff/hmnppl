/**
 * Next.js middleware G현 security headers + Supabase Auth session management.
 *
 * Responsibilities:
...
export async function middleware(request: NextRequest) {
  ...
  // 5. Authenticated user G현 Onboarding Enforcement ------------------------------
  if (user && !isPublicRoute(pathname)) {
  ...
  // 6. Authenticated user G현 Layer 1 RBAC: role-based route protection ----------
  //    Uses the cached role cookie for a fast, coarse-grained check.
  //    If the cookie is missing, we allow through G현 Layer 2 (API) or the
  ...
  // 7. Authenticated user G현 enforce session timeouts ----------------------------
  if (user) {
    const now = Date.now();
...
// ---------------------------------------------------------------------------
// Matcher G현 run on every request except static assets
// ---------------------------------------------------------------------------
